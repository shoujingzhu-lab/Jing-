...
class DataBroadcaster:
    ...
    async def _keep_cache_warm_loop(self):
        """后台缓存保温 — 每 15s 刷新所有常用交易对的缓存，确保 REST API 始终命中"""
        POPULAR = ["BTC/USDT", "ETH/USDT"]
        while self._running:
            try:
                for symbol in POPULAR:
                    for exchange in market_data_service.SUPPORTED_EXCHANGES:
                        try:
                            timeout = 30.0 if exchange == "gateio" else 5.0
                            await asyncio.wait_for(
                                market_data_service.get_ticker(exchange, symbol),
                                timeout=timeout,
                            )
                            self._exchange_available[exchange] = True
            ...
            await asyncio.sleep(15)  # 每 15s 刷新一次，缓存 TTL 30s 不会过期
